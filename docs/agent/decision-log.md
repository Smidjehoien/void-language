...
| **Adopted** | Use one authoritative deterministic plan with units `{ordinal, query, platform}`. | Reject non-manifest platforms, then plan every requested unit in normalized-input × pinned-manifest order without query/compatibility filtering. `plannedUnits = actualQueryCount * platformCount` binds approval/HMAC state, attempts, ordinals, and report requested counts; omitted provider results become `unknown`. No LLM planner is required. |
...
| **Adopted** | Retry one provider check and never replay a success. | Provider-lane pacing, pause, retry-after, and circuits contain amplification. Total attempts are bounded by `plannedUnits * (1 + retriesPerUnit)` plus preset/manifest/active-budget ceilings. |
| **Adopted** | Use canonical `executionMode`, strictly local dry-run, and plan-bound approvals. | `--approve-live` authorizes rather than overrides. Dry-run validates and plans locally, emits only bounded `planned` counts/limits, and performs no subprocess, adapter, secret-channel, provider/network, dispatch, or progress-checkpoint activity. Compatibility probes are separate. Live approvals bind plan/version/count/platforms/preset/attempts/expiry/operator; Fast needs separate acknowledgment. |
| **Adopted** | Support privacy-safe v1 resume with one active processing budget. | Opaque run IDs, exclusive locks, deterministic ordinals, aggregate-only HMAC-bound checkpoints, and authenticated non-increasing `remainingActiveBudgetMs` permit skip-without-replay across reboot. Each invocation derives a monotonic deadline; dormant time is free, pre-attempt reservation bounds crashes, budget never resets/increases, and fresh live/Fast approval remains required. |
...
| **Adopted** | Enforce active-budget deadlines, secure local paths, run-owned checkpoint replacement, and atomic no-clobber report publication. | Cancellation covers all phases and process groups, with a 1-second graceful termination and 2-second cleanup/report reserve. Reports use a same-directory temporary plus atomic no-replace publication, fail closed if unavailable, never overwrite, and return `REPORT_DESTINATION_EXISTS` on a destination race; other required publication failures return `REPORT_WRITE_FAILED`. |
